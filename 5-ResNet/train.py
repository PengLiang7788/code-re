...
# 创建实验结果文件夹路径
exp_path = "./runs/report/{}/{}".format(args.model_name, exp_name)
...
# 创建数据集
train_set = MyDataset(args.dataset, mode="train", transform=data_transform["train"], split_ratio=0.9,
                      rng_seed=args.seed)
...
if __name__ == '__main__':
    tb_path = "runs/logs/{}/{}".format(args.model_name, args.exp_postfix)
    tb_writer = SummaryWriter(log_dir=tb_path)

    lr = args.lr
    model = create_model(model_name=args.model_name, num_classes=args.classes_num)
    model.to(device)
